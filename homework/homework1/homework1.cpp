...
public:
	...
	struct Vertex {
		...
		glm::vec3 color;
        glm::vec3 tangent;
        uint32_t nodeIndex; // 记录这个顶点在哪个节点下，用于查询顶点动画变化状态
	};
	...
	*/
	...
	~VulkanglTFModel()
	{
        ...
        for(auto& mat : materials){
            mat.materialFactors.buffer.destroy();
        }
	}
	...
	void loadMaterials(tinygltf::Model& input)
	{
		...
		for (size_t i = 0; i < input.materials.size(); i++) {
			...
			if (glTFMaterial.values.find("baseColorFactor") != glTFMaterial.values.end()) {
				materials[i].materialFactors.values.baseColorFactor = glm::make_vec4(glTFMaterial.values["baseColorFactor"].ColorFactor().data());
			}
		...
		if (node->mesh.primitives.size() > 0) {
			...
			for (VulkanglTFModel::Primitive& primitive : node->mesh.primitives) {
				if (primitive.indexCount > 0) {
                    ...
                    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 3, 1, &mat.materialFactors.descriptorSet, 0, nullptr);
...
public:
	...
	struct descriptorSetLayouts {
		...
		//VkDescriptorSetLayout samplerColorMap;
		//VkDescriptorSetLayout samplerNormalMap;
		//VkDescriptorSetLayout samplerMetalRoughMap;
		//VkDescriptorSetLayout samplerEmissiveMap;
	...
	void setupDescriptors()
	{
		...
		*/
		...
		// Pipeline layout (set 0 = matrices, set 1 = textures, set 2 = animTrs, set 3 = materials)
        ...
        for (auto& mat : glTFModel.materials){
            ...
            VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &allocInfo, &mat.materialFactors.descriptorSet));
            std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
                vks::initializers::writeDescriptorSet(mat.materialFactors.descriptorSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &mat.materialFactors.buffer.descriptor),
            };
            vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, nullptr);
        }
	...
	void prepareUniformBuffers()
	{
        ...
        for(auto& mat : glTFModel.materials)
        {
            VK_CHECK_RESULT(vulkanDevice->createBuffer(
                ...
                &mat.materialFactors.buffer,
                sizeof(VulkanglTFModel::MaterialFactor::Values),
                &mat.materialFactors.values));
            
            VK_CHECK_RESULT(mat.materialFactors.buffer.map());
        }
